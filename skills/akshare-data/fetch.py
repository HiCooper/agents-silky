...
#    2026-09-17 交叉校验：13:43 累计 14,377.4 亿、当日官方全天待公布，
#    按本表 13:43→0.806 反推半日系数应约 **0.70**（原 0.66 偏低）。
# ⚠️⚠️ 但**这张表本身就是估计**（实测样本 n=2）：口径误差可达 ±5~6%，
#    在「与昨日基本持平」附近**足以翻转「放量 / 缩量」的定性结论**。
#    因此 `cmd_turnover` 输出的是**区间**；**凡定性结论（放量/缩量、强/弱）
#    不得单独建立在推算值上，必须等收盘官方口径**。
_CUM_SHARE = [(9 * 60 + 30, 0.00), (10 * 60, 0.30), (10 * 60 + 30, 0.45), (11 * 60, 0.56),
              (11 * 60 + 30, 0.70), (13 * 60, 0.70), (13 * 60 + 30, 0.78),
              (14 * 60, 0.84), (14 * 60 + 30, 0.92), (15 * 60, 1.00)]
# 口径不确定度：±0.04（占全天比例），用于把点估计改写成区间
_SHARE_BAND = 0.04
...
def cmd_turnover(args=None):
    ...
    if sub in ("now", "盘中", "实时", ""):
        ...
        try:
            ...
            if share > 0:
                lo = (sh + sz) / min(1.0, share + _SHARE_BAND)
                hi = (sh + sz) / max(0.01, share - _SHARE_BAND)
                print(f"  已成交时段占全天约 {share * 100:.0f}%（按典型日内分布，口径误差 ±{_SHARE_BAND * 100:.0f}pp）："
                      f"全天或约 **{lo:,.0f} ~ {hi:,.0f} 亿**（成交额前重后轻，非等比外推）")
                print("  ⚠️ 这是**推算区间**，不是官方值：距昨日 ±5% 以内时**不足以判定放量/缩量**，"
                      "定性结论请等 `fetch turnover eod`。")
        except Exception as e:
