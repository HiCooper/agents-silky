...
> **路径约定**：`$SKILLS` = 本机 skill 仓库根 `/Users/xueancao/Projects/QoderProjects/agents-silky/skills`。本 skill 的脚本在 `$SKILLS/economic-analysis-expert/scripts/`；`akshare-data` 的入口在 `$SKILLS/akshare-data/fetch`；下文 `$PY` = `$SKILLS/akshare-data/.venv/bin/python`。

---

## 1. 快讯（**财联社电报首选** + 格隆汇补充）

### 1.1 财联社电报（`akshare-data/fetch news`，首选）

akshare `stock_info_global_cls(symbol=...)`，走财联社当前网页端电报接口，实测 **0.1s**、稳定、无需鉴权。

```bash
$SKILLS/akshare-data/fetch news           # **默认「重点」频道**（只看重点）
$SKILLS/akshare-data/fetch news 成交额      # 在重点频道里按关键词过滤
$SKILLS/akshare-data/fetch news 全部        # 切到全部频道（最近 20 条），可再跟关键词
```

- **默认只取「重点」频道**（条数少、噪声低，是催化剂扫描的首选）；`全部` 只在需要搜全量关键词时显式指定。
...
## 2. A股指数 / ETF / 美股 / 债券（**首选走现成 `akshare-data` skill**）

> **入口优先级（2026-09-15 起）**：**A 股指数/ETF 与美股一律先走 `$SKILLS/akshare-data/fetch`**。它拉不到的（跨市场实时、商品/汇率、韩国、美股盘中价、历史日K）才回本 skill 的 `scripts/` 与本文 §3、§4。

**akshare 只装在 skill 自带 venv 里，系统 `python3` 没有。**

```bash
# —— A股 / 债券 ——
$SKILLS/akshare-data/fetch indices        # 核心指数一览（上证/深成/创业板/科创50/沪深300/上证50/中证500/中证1000），实测 ~3s
$SKILLS/akshare-data/fetch index 000688   # 单个指数（数字自动补 sh/sz；给开/高/低/昨收/成交额）
$SKILLS/akshare-data/fetch index 科创50    # 支持名称关键词
$SKILLS/akshare-data/fetch etf 半导体       # 按关键词搜 ETF（⚠️ 全市场扫描，实测 18–20s）
$SKILLS/akshare-data/fetch etf 512480      # 按代码查 ETF（同样 ~18s）
$SKILLS/akshare-data/fetch bond            # 中债收益率（EOD，滞后一日）；⚠️ 美债列时通时不通
$SKILLS/akshare-data/fetch gbond           # 全球国债收益率 EOD（美/中/日/德/英/法/意；akshare 未封装日德）
$SKILLS/akshare-data/fetch margin          # 融资融券因子（沪深北余额/1-5-20日变动/维持担保比例）
$SKILLS/akshare-data/fetch margin top 15   # 个股融资余额排行（拥挤度第二维：谁最容易被强平）
$SKILLS/akshare-data/fetch turnover        # 两市成交额（**指数法**：沪+深+北证50；实时 / eod / hist）
...
$SKILLS/akshare-data/fetch a50             # A50 期指（东财外盘期货源，全期限 + 持仓量，★ 标主力）

# —— 美股（日线，新浪源）——
$SKILLS/akshare-data/fetch us              # 标普/道指/纳指/费半 .SOX，实测 ~3s
$SKILLS/akshare-data/fetch us semis        # 预设半导体篮子 AVGO/NVDA/TSM/AMD/ASML/INTC
$SKILLS/akshare-data/fetch us stock AVGO NVDA MU   # 任意美股代码，不限篮子
```
...
```bash
$SKILLS/akshare-data/.venv/bin/python -c "import akshare as ak; ..."
```
...
**首选 `$SKILLS/akshare-data/fetch us [semis|stock <代码...>]`**（A 股与美股统一入口，见 §2）。本 skill 的 `scripts/us_data.py` 是**同一取数逻辑的本地封装**，只在需要在 Python 里内嵌调用时才用（两者实测结果一致，任选其一，别两套混着报数）。
...
### 3.1 多只 A 股个股「全量字段」批量取（`akshare-data` 不覆盖的场景）
...
> **边界（别搞混）**：
> - **A 股指数/ETF** 一律先走 `akshare-data/fetch index|etf`（见 §2）；`quote.py cn` 只负责它**不覆盖**的「多只个股一次拿齐开/昨收/收/高/低/额」，收到指数/ETF 代码会打印提示。
> - **A50 期指** 先走 `akshare-data/fetch a50`（东财源，含全期限与持仓量）；`quote.py` 的 `hf_CHA50CFD` 只是**东财被挡时的免 venv 兜底**（单合约 CFD）。
...
python3 $Q cn 300308 300502 601138   # A股**个股批量**（指数/ETF 请走 akshare-data，见上）
...
### 3.3 分时（日内走势）——如何拿「迷你走势图 / 日内形态」的数据

两个**新浪原始端点**（都不在 akshare 里，实测 2026-09-15 全部可用）：
...
⚠️ **看到形态 ≠ 可以把盘中跳动当趋势**：`premarket-korea-factor.md` §4 的「趋势 > 跳动」照样成立——9-14 的教训就是拿韩国 10:00 的反弹当企稳，结果当天韩国收在近全天最低。

💡 **时点差异很关键**：韩股 14:30（北京）后分时**定格**，而纳指期货**近 24 小时连续**（实测当天 06:01→21:42 共 939 点）。所以——**盘前与夜间要看「活着的」跨市场读数时，纳指期货优先于韩国**；韩国只在 08:00–14:30 这个窗口内是最新鲜的亚洲芯片读数。
...
- **全球国债收益率的「实时」值**（新浪 `gb_$tnx` 不可用）→ WebSearch。**先分清实时还是 EOD**：
  **EOD 收盘值走本地**——`akshare-data/fetch gbond`（美/中/日/德/英/法/意，含日变动与 5 日变动，实测 2026-09-14 收盘：美 10Y 4.983%、日 10Y 2.999%、德 10Y 3.521%、英 10Y 5.370%、中 10Y 1.686%）；**只有盘中实时值才需要 WebSearch**。
...
- [ ] **A 股指数/ETF 与美股，是否先走了 `$SKILLS/akshare-data/fetch`**？（别一上来就手写 akshare、或绕开现成入口）
...
- [ ] 跨市场判断是否**多个来源交叉**（akshare-data + market_panel + WebSearch），而不是单点？
