...
name: ashare-data
version: 1.3.0
description: Fetch A-share index quotes (上证指数/深证成指/创业板指/科创50/沪深300等), ETF quotes (半导体/芯片/科技/科创50等), China/US/Japan/Germany/UK treasury yields (global bond via Sina), A50 index futures, and US index/stock daily quotes (标普/道指/纳指/费城半导体SOX、AVGO/NVDA/TSM等美股) via akshare. Use when the user asks for 指数点位、指数涨跌、A股行情、ETF行情、半导体ETF、国债收益率(10年美债/中债/日债/德债/英债)、美股行情/美股指数/费半/隔夜美股收盘，或需要这些标的的实时/最新行情数据.
name_zh: A股/美股行情数据
...
4. **日/德国债 akshare 没封装，走的是新浪端点**：akshare 只有中/美（`bond_zh_us_rate`）和**美国各期限**（`bond_gb_us_sina`，symbol_map 仅列美国）。日本/德国/英国等的底层是同一处新浪全球国债端点 `bond.finance.sina.com.cn/hq/gb/daily?symbol=JP10YT`，实测 `US/CN/JP/DE/GB/FR/IT` 各国各期限全通（约 1000 条日线），**`fetch gbond` 已封装**。所以「日债/德债能不能拿」的答案是**能，但别去 akshare 找**。
